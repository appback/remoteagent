...
process.exit(0);
`;
  for (const name of ['systemctl','systemd-run','systemd-analyze','sudo','loginctl']) await fs.writeFile(path.join(bin,name),mock,{mode:0o700});
  ...
  const verify=spawnSync('/usr/bin/systemd-analyze',['verify',path.join(temp,'.config/systemd/user/remoteagent.service')],{encoding:'utf8'});
